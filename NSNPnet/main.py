...
def validate(val_loader, model, criterion, args,best_acc):
    ...
    with torch.no_grad():
        ...
        if top1.avg > best_acc:  # and top1.avg>=66:
            confusion_matrix.plot_confusion_matrix_2(pres_tr,trues_tr)
            if True:  # top1.avg>=66:
        ...
        print('Current UAR: '+ str(confusion_matrix.plot_confusion_matrix_2(pres_tr,trues_tr)))
        with open(log_txt_path, 'a') as f:
            f.write('Current Accuracy: {top1.avg:.3f}'.format(top1=top1) + '\n')
            f.write('Current UAR: '+ str(confusion_matrix.plot_confusion_matrix_2(pres_tr,trues_tr)) + '\n')
    return top1.avg, losses.avg
