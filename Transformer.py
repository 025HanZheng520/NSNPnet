...
from torch.nn import Conv1d,ModuleList
...
class FFNWithPreNorm(nn.Module):
    def __init__(self, embed_dims=256, hidden_channels=1024, num_layers=2, act_layer=nn.GELU, dropout_p=0,layer_drop=None,norm_layer=nn.LayerNorm,):
        super().__init__()
        self.embed_dims = embed_dims
        ...
        for _ in range(num_layers - 1):
            ...
            conv_layer = nn.Conv1d(in_channels, hidden_channels, kernel_size=3, stride=1, padding=padding,
                                   dilation=dilation)
    ...
    def forward(self, x):
        ...
        residual=x #[32,785,512]

        # FFN
        x = self.norm(x)  # ([32, 783, 512])
        for layer in self.layers:
            x = layer(x)  # ([32, 783, 2048])  track 512*2048
        # 将输入x的shape从[batch_size, seq_len, embed_dims]转换为[batch_size, embed_dims, seq_len]
        x = x.permute(0, 2, 1) #[32,512,785]

        # TCN
        for i in range(self.num_layers - 1):
            x = self.act_layer()(self.conv_layers[i](x))#[32,2048,783]
            x = self.dropout(x)#[32,2048,783]
        # 将输出x的shape从[batch_size, embed_dims, seq_len]转换为[batch_size, seq_len, embed_dims]
        x = x.permute(0, 2, 1)#[32,783,2048]
        x = self.linear(x)  # [32,783,512]
        x = self.dropout(x)  # 0.1[32,783,512]->([32, 781, 512])
        #zero_tensor = torch.zeros(32, 2, 512)
        #zero_tensor = torch.zeros_like(x[:, :2, :])  # 创建一个与 x 张量相同形状的零张量
        mean_tensor = torch.mean(x[:, :2, :].clone())
        shape = x[:, :2, :].shape
        mean_tensor = torch.ones_like(x[:, :2, :]) * mean_tensor
        #zero_tensor=zero_tensor.to("cuda:0")
        mean_tensor=mean_tensor.to("cuda:0")
        x = torch.cat((x, mean_tensor), dim=1)





        return residual + self.layer_drop(x)#[32,785,512]+#[32,783,512]
